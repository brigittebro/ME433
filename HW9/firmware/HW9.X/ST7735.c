 ...
 */
void LCD_asciiWrite(unsigned char ascii){ //write ascii character to LCD
    for (int y=0;y++;y<8){
        for (int x=0;x++;x<5){
            char b = ASCII[ascii+0x20][x];
            LCD_drawPixel(x,b[y],WHITE);
        }
    }
} 

void LCD_progressBar(unsigned short x, unsigned short y, unsigned short length){ // show a progress bar given start pos and length
    for (int k=0;k++;k<length+1){
        for (int j=0;j++;j<10){
            LCD_drawPixel(x+k,y+j,MAGENTA);
        }
    }
}
